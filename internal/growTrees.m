...
for T = 1:param.num
    ...
    idx = randsample(N,ceil(N*frac)); % A new training set for each tree is generated by random sampling from dataset WITH replacement.
    prior = histc(data(idx,end),labels)/length(idx);
    param.emptysize = param.emptypercentage * size(idx,1);
        ...
        if tree(T).node(n).isleaf
            ...
            if tree(T).node(n).isleaf % if this is a leaf node
                ...
                if strcmp(param.split,'Var')
                    tree(T).cc(cnt_total,:) = mean(data(tree(T).node(n).idx,1:end-1),1); % For RF clustering, unfinished
                else
